/* QRCode core by Kazuhiko Arase, MIT license. Browser bundle for Battrochtek. */
(function(global){
  "use strict";
  const modules={
...
module.exports = QRCode;
  ...
  function localRequire(id){
    ...
    const module={exports:{}}; cache[id]=module; modules[id](module,module.exports,localRequire); return module.exports;
  }
  const QRCode=localRequire('./index');
  const Level=localRequire('./QRErrorCorrectLevel');
  ...
  function renderCanvas(canvas,text,opts){
    ...
    const size=Number.isFinite(opts.size)?opts.size:280;
    ...
    ctx.fillStyle=opts.light||'#ffffff'; ctx.fillRect(0,0,size,size);
    const moduleSize=size/total; ctx.fillStyle=opts.dark||'#0b1118';
    for(let r=0;r<matrix.length;r++) for(let c=0;c<matrix.length;c++) if(matrix[r][c]) {
      const x=Math.floor((c+quiet)*moduleSize); const y=Math.floor((r+quiet)*moduleSize);
      const x2=Math.ceil((c+quiet+1)*moduleSize); const y2=Math.ceil((r+quiet+1)*moduleSize);
      ctx.fillRect(x,y,x2-x,y2-y);
    }
    return matrix.length;
  }
  global.BtQRCode={makeMatrix,renderCanvas};
})(window);
